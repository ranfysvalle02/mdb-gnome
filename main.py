...
async def _register_experiments(app: FastAPI, active_cfgs: List[Dict[str, Any]], *, is_reload: bool = False):  
    ...
    for cfg in active_cfgs:  
        ...
        if not exp_path.is_dir():  
            logger.warning(f"[{slug}] Skipped: local Thin Client directory not found.")  
            continue  
        ...
        try:  
            ...
            logger.warning(f"[{slug}] No local actor module found. Skipped.")  
            continue  
        except Exception as e:  
        ...
        try:  
            ...
            logger.warning(f"[{slug}] No local '__init__.py'? Skipped.")  
            continue  
        except Exception as e:  
