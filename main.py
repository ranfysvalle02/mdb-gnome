...
async def lifespan(app: FastAPI):  
    ...
    if RAY_AVAILABLE:  
        # By request:  
        # If environment_mode == "shared", use an in-process or embedded local cluster  
        # Otherwise, use RAY_ADDRESS env var (or "auto") for external connection  
        if app.state.environment_mode == "shared":  
            RAY_CONNECTION_ADDRESS = "local"  
        else:  
