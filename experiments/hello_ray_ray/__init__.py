...
if not TEMPLATE_DIR.is_dir():
    logger.critical(f"[HelloRay] Template directory NOT FOUND at: {TEMPLATE_DIR}")
if not INDEX_HTML_FILE.is_file():
    logger.critical(f"[HelloRay] 'index.html' NOT FOUND at: {INDEX_HTML_FILE}")
...
async def demo_ray_index(request: Request):
    ...
    if not INDEX_HTML_FILE.is_file():
        logger.error(f"[HelloRay] Cannot render: 'index.html' not found at {INDEX_HTML_FILE}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    logger.info("[HelloRay] Getting actor handle...")
    actor = get_actor_handle(request)
    logger.info(f"[HelloRay] Got actor handle: {actor} (type: {type(actor)})")

    # 3. Call the actor method
    logger.info("[HelloRay] Calling actor.say_hello.remote()...")
    try:
        greeting = await actor.say_hello.remote()
        logger.info(f"[HelloRay] Actor call completed. Greeting: {greeting}")
    except Exception as e:
        logger.error(f"[HelloRay] Actor call 'say_hello' failed: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
